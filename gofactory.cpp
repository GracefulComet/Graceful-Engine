...
//void GOfactory::updateGO(int key, SDL_Event Event, float deltaT) {
  // Goes through a specific player  and updates their internal state by
  // speficifying the specific player key
//  GOList[key].update(Event, deltaT);
//}

//void GOfactory::updateAllGO(SDL_Event Event, float deltaT) {
  // loops through each GameObject and and updates its internal state till all
  // GameObjects are updated
...
//void GOfactory::drawAllGO(SDL_Renderer *pRender) {
  // loops through each player and draws it at its position untill each player
  // is drawn
//  for (int i = 1; i <= P_Num_Keys; i++) {
//    drawGO(i, pRender);
//  }
...
//void GOfactory::addGO(std::string Textureid, int NumCol, int NumRow, int Width,
...
//  Sprite temp;
 ...
 // GOList[key] = temp;
//}
//void GOfactory::Anim(float duration, int key, int NumFrames,
 //                    SDL_Renderer *pRender) {

//  GOList[key].Nanimate(duration, NumFrames, pRender);
//}
//void GOfactory::AddGrid(int NumCol, int NumRows, std::string textureID,
//                        int width, int height, int GridX, int GridY) {
